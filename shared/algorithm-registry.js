import { roundedScore } from './common-utils.js';

export const ALGORITHM_AUDIT_KEYS = [
  'globalStage', 'localStage', 'baseStage', 'ratio', 'radial', 'hu',
  'fourier', 'efd', 'structural', 'angle', 'fill', 'minutiae',
  'localFeature', 'advanced', 'advancedRaw', 'ratioGate', 'localGate',
  'hausdorff', 'shapeContext', 'icp', 'ransac', 'zernike'
];
...
export function algorithmVerdict(delta) {
...
export function buildAlgorithmAudit(best, expectedCandidate, expectedProfile) {
  ...
  const rows = ALGORITHM_AUDIT_KEYS.map(key => {
    ...
    const delta = expectedFound && Number.isFinite(expectedScore) && Number.isFinite(bestScore)
