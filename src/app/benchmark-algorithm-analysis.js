export const BENCHMARK_ALGORITHM_KEYS = [
  'globalStage', 'localStage', 'baseStage', 'ratio', 'radial', 'hu',
  'fourier', 'efd', 'angle', 'fill', 'minutiae', 'localFeature', 'structural',
  'advanced', 'advancedRaw', 'ratioGate', 'localGate', 'hausdorff',
  'shapeContext', 'icp', 'ransac', 'zernike'
];

export function buildAlgorithmAudit(best, expectedCandidate) {
  const rows = BENCHMARK_ALGORITHM_KEYS.map(key => {
...
export function buildAlgorithmRanks(candidates, expectedReference, bestReference) {
  return BENCHMARK_ALGORITHM_KEYS.map(key => {
...
export function summarizeAlgorithmEffectiveness(results) {
  ...
  }).sort((left, right) => (right.effectivenessScore ?? -1) - (left.effectivenessScore ?? -1));
}
